...
```
python extract_BCs.py --fq1 <fastq R1 file> --fq2 <fastq R1 file> --outdir <output path> --outprefix <output prefix>
```
...
```
./consensus_linBC.sh <path to .clustered.txt>
```
...
<b>Step 6</b>: Clonal families are inferred by constructing a barcode-similarity graph in NetworkX, cells with a barcode set Jaccard similarity of ≥0.3 are considered as pairs, and connected components are defined as a single clonal family.
 This script generates a <i>.gml</i> file
```
python clonal_graph.py <.linBC_matrix.csv> <.linBC_matrix.csv> <jaccard threshold>
```

<b>Step 7</b>
Clonal families are extracted from the .gml file. This script generates a <i>.clones.csv</i> file

```
python clonal_families.py <gml file>
```

<b>Step 8</b>: Append clonal information from step 7 to the adata h5ad file generated in step 1
```
